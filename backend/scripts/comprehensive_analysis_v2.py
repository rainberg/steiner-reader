"""Comprehensive analysis v2: 改进假阳性检测，特别是模糊匹配定位错误模式。

关键改进:
1. 检测同一GA的overlap/order_bad聚集+差距递增 → 模糊匹配定位错误
2. 间隙>100000字符 → 很可能是定位错误而非真实缺失
3. 更细致的first/last_not_found分类
"""
import json
import re
...
OUTPUT_JSON = "d:/00_dev/Codex/Steiner_Reader_Web/comprehensive_analysis_v2.json"
OUTPUT_TXT = "d:/00_dev/Codex/Steiner_Reader_Web/comprehensive_analysis_v2.txt"
...
def parse_positions(desc):
    """从description解析位置信息"""
    # order_bad: "First pos (X) >= last pos (Y)"
    m = re.search(r"First pos \((\d+)\) >= last pos \((\d+)\)", desc)
    if m:
        return int(m.group(1)), int(m.group(2))
    # overlap: "Lecture starts at X but previous ended at Y"
...
def classify_first_not_found(issue):
    ...
    return "P1-real_issue", "real_content", f"真实首句差异"
...
def classify_last_not_found(issue):
    ...
    return "P1-real_issue", "real_content", f"真实末句差异"


def detect_matching_error_pattern(ga_issues):
    """检测模糊匹配定位错误模式：同一GA有多个overlap/order_bad且位置聚集"""
    overlap_positions = []
    order_bad_positions = []

    for issue in ga_issues:
        if issue["type"] == "overlap":
            start, prev_end = parse_positions(issue.get("description", ""))
    ...
    # 如果同一GA有≥5个overlap且首句位置都聚集在PDF开头（前5000字符内），是定位错误
    ...
    # 如果同一GA有≥5个order_bad且首句位置都相同或聚集
    if len(order_bad_positions) >= 5:
        if len(set(order_bad_positions)) <= 3:  # 位置几乎都相同
...
def classify_order_bad(issue, is_matching_error):
    ...
    if is_matching_error:
        return "P3-false_positive", "matching_error", "模糊匹配定位错误（同一GA多个order_bad聚集）"
    ...
    if first_pos is not None and last_pos is not None:
        ...
        if diff < 100:
            return "P3-false_positive", "tiny_diff", f"位置差距极小({diff}字符)"
        if diff < 1000:
            return "P2-needs_review", "small_diff", f"位置差距较小({diff}字符)"
        return "P1-real_issue", "large_diff", f"位置差距大({diff}字符)"
...
def classify_overlap(issue, is_matching_error):
    """分类 overlap 问题"""
    if is_matching_error:
        return "P3-false_positive", "matching_error", "模糊匹配定位错误（同一GA多个overlap聚集）"
...
def classify_gap(issue):
    ...
    # 间隙>100000字符：很可能是定位错误（末句定位错误导致间隙异常大）
    if gap_size > 100000:
        return "P3-false_positive", "huge_gap_matching_error", f"间隙异常大({gap_size}字符)，定位错误"
...
def main():
    ...
    ga_matching_error = {}  # GA -> (is_error, pattern_type)
    ...
    with open(OUTPUT_TXT, "w", encoding="utf-8") as f:
        ...
        f.write("   GA: " + ", ".join(sorted(set(c['ga'] for c in by_priority['P0-missing_lecture']))) + "\n")
        ...
        f.write(f"   含 {len(ga_matching_error)} 个GA的模糊匹配定位错误\n")
        f.write("   行动: 改进验证脚本的匹配策略\n\n")

        f.write(f"5. 【单独处理】P4 - {len(by_priority['P4-skip'])}个跳过\n")
        f.write("   行动: 调查steiner.wiki正确URL格式\n")
