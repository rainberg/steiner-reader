...
${sentencesHtml}
${contributions.length > 0 ? '<div class="contributors" style="margin-top:2em;padding:1em;border-top:1px solid #e5e7eb"><h3 style="font-size:0.8em;color:#94a3b8;margin-bottom:0.5em">贡献者</h3>' + contributions.map(c => '<span style="font-size:0.75em;color:#94a3b8;margin-right:1em">' + c.username + ' (' + (c.contribution_type === 'first_translation' ? '首次翻译' : c.contribution_type === 'revision' ? '修订' + (c.count > 1 ? ' ×' + c.count : '') : '下载') + ')</span>').join('') + '</div>' : ''}
            ...
            <div className="flex flex-wrap gap-2">
              {contributions.map((c, i) => (
                <span key={i} className="text-xs bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full">
                  {c.username} ({c.contribution_type === 'first_translation' ? '首次翻译' : c.contribution_type === 'revision' ? `修订${c.count > 1 ? ' ×'+c.count : ''}` : '下载'})
                </span>
              ))}
            </div>
          </div>
