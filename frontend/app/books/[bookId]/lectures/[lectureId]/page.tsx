...
export default function LecturePage() {
  ...
  const loadLecture = useCallback(async () => {
    try {
      ...
      if (cost) {
        setCostInfo(cost);
        if (cost.user_credits !== null) { setUserCredits(cost.user_credits); updateStoredCredits(cost.user_credits); }
      }
      if (perm) {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      if (res.status === 'already_translated') {
        ...
        if (res.credits !== undefined) { setUserCredits(Number(res.credits)); updateStoredCredits(Number(res.credits)); }
        startPolling();
      }
    } catch (err: unknown) {
      setTranslating(false);
...
${sentencesHtml}
${contributions.length > 0 ? '<div class="contributors" style="margin-top:2em;padding:1em;border-top:1px solid #e5e7eb"><h3 style="font-size:0.8em;color:#94a3b8;margin-bottom:0.5em">贡献者</h3>' + contributions.map(c => '<span style="font-size:0.75em;color:#94a3b8;margin-right:1em">' + (c.display_name || '匿名') + ' (' + (c.contribution_type === 'translate' ? '翻译' : c.contribution_type === 'download_purchase' ? '下载' : c.contribution_type) + ')</span>').join('') + '</div>' : ''}
<div class="notice">Generated by Steiner Reader — 仅供个人学习使用</div>
  ...
  const handleEditSentence = async (sentenceId: number) => {
    ...
    try {
      ...
      updateStoredCredits(res.credits_remaining);
            ...
            <div className="flex flex-wrap gap-2">
              {contributions.map((c, i) => (
                <span key={i} className="text-xs bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full">
                  {c.display_name || '匿名'} ({c.contribution_type === 'translate' ? '翻译' : c.contribution_type === 'download_purchase' ? '下载' : c.contribution_type})
                </span>
              ))}
            </div>
          </div>
