...
export default function LecturePage() {
  ...
  const loadLecture = useCallback(async () => {
    try {
      ...
      try {
        const status = await getTranslationStatus(lectureId);
        if ((status.is_running || status.is_translating) && !status.completed) {
          setTranslating(true);
          startPolling();
        }
      } catch {
  ...
  const handleDownload = async () => {
    try {
      const blob = await downloadLecturePdf(lectureId);
      const title = lecture?.title_de || 'Lecture';
      const safeTitle = title.replace(/[^a-zA-Z0-9\u4e00-\u9fff _-]/g, '').slice(0, 40).trim();
