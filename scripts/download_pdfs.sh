...
for i in "${!GA_IDS[@]}"; do
    GA_ID="${GA_IDS[$i]}"
    FILENAME="GA${GA_ID^^}.pdf"
    FILEPATH="$PDF_DIR/$FILENAME"
    URL="http://bdn-steiner.ru/cat/ga/${GA_ID}.pdf"
    ...
    HTTP_CODE=$(curl -s -f -o "$FILEPATH" -w "%{http_code}" \
        --connect-timeout 15 --max-time 120 "$URL" 2>/dev/null || echo "000")
    
    if [ "$HTTP_CODE" = "200" ]; then
        SIZE=$(stat -c%s "$FILEPATH" 2>/dev/null || echo 0)
        if [ "$SIZE" -gt 1000 ]; then
            SUCCESS=$((SUCCESS + 1))
            echo "[${i+1}/${TOTAL}] OK GA${GA_ID} (${SIZE} bytes)"
        else
            rm -f "$FILEPATH"
            NOT_FOUND=$((NOT_FOUND + 1))
            echo "[${i+1}/${TOTAL}] MISS GA${GA_ID} (too small)"
        fi
    else
        rm -f "$FILEPATH"
        NOT_FOUND=$((NOT_FOUND + 1))
        echo "[${i+1}/${TOTAL}] MISS GA${GA_ID} (HTTP ${HTTP_CODE})"
    fi
