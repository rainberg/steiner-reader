#!/bin/bash
# Download all GA PDFs in parallel from bdn-steiner.ru
PDF_DIR="$HOME/steiner-reader/data/pdf"
mkdir -p "$PDF_DIR"

# Generate list of GA IDs
GA_IDS=(
...
download_one() {
    local GA_ID="$1"
    local FILENAME="GA${GA_ID^^}.pdf"
    local FILEPATH="$PDF_DIR/$FILENAME"
    local URL="http://bdn-steiner.ru/cat/ga/${GA_ID}.pdf"
    
    # Skip if already downloaded and > 100KB
    if [ -f "$FILEPATH" ] && [ $(stat -c%s "$FILEPATH" 2>/dev/null || echo 0) -gt 100000 ]; then
        echo "SKIP GA${GA_ID}"
        return 0
    fi
    
    local HTTP_CODE=$(curl -s -f -o "$FILEPATH" -w "%{http_code}" \
        --connect-timeout 15 --max-time 300 "$URL" 2>/dev/null || echo "000")
    
    if [ "$HTTP_CODE" = "200" ]; then
        local SIZE=$(stat -c%s "$FILEPATH" 2>/dev/null || echo 0)
        if [ "$SIZE" -gt 1000 ]; then
            echo "OK GA${GA_ID} (${SIZE} bytes)"
        else
            rm -f "$FILEPATH"
            echo "MISS GA${GA_ID} (too small)"
        fi
    else
        rm -f "$FILEPATH"
        echo "MISS GA${GA_ID} (HTTP ${HTTP_CODE})"
    fi
}
...
printf '%s\n' "${GA_IDS[@]}" | xargs -P 5 -I {} bash -c 'download_one "$@"' _ {}
