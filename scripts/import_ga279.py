#!/usr/bin/env python3
"""
Import GA279 (Eurythmie als sichtbare Sprache) into PostgreSQL database.

Parses the PDF, extracts 17 lectures with paragraphs and sentences,
and inserts them into the steiner_reader database.
"""
import re
import fitz  # PyMuPDF
import psycopg2
...
def printed_to_pdf_page(printed_page):
    ...
    # Offset: printed_page = pdf_page_index + 3
    # So pdf_page_index = printed_page - 3
...
def split_into_paragraphs(text):
    ...
    # Clean up: strip whitespace, filter short/empty
...
def split_into_sentences(text):
    ...
    # German sentence splitting - handle abbreviations and special cases
...
def clean_lecture_text(text, title):
    ...
    # Remove the lecture title that appears at the top of the first page
    lines = text.split('\n')
    cleaned_lines = []
    skip_header = True
    ...
    for line in lines:
        if skip_header:
            # Skip lines that look like the lecture title
            stripped = line.strip()
            ...
            if not stripped:
                skip_header = False
                continue
            # If we've passed the title lines, stop skipping
            skip_header = False
...
def create_book(cursor):
    """Insert the GA279 book record."""
    # Check if book already exists
    cursor.execute("SELECT id FROM books WHERE ga_number = %s", ("GA279",))
    row = cursor.fetchone()
    if row:
        book_id = row[0]
        print(f"    Book already exists with ID: {book_id}")
...
def main():
    ...
    try:
        ...
        print(f"\n[3] Creating/updating book record...")
        book_id = create_book(cursor)
        print(f"    Book ID: {book_id}")
        
        # 4. Delete existing lectures for this book (in case of re-import)
        cursor.execute("DELETE FROM lectures WHERE book_id = %s", (book_id,))
        deleted = cursor.rowcount
        print(f"    Deleted existing lectures: {deleted}")
        
        # 5. Import each lecture
        ...
        for lecture_info in LECTURES:
            ...
            # Count sentences for this lecture
            cursor.execute("""
